...
class Recommendation:
    ...
    def Agglomerative_CompleteLink_cluster_Visitors(
        self, X: pd.DataFrame, number_of_clusters: int
    ):
        return AgglomerativeClustering(
            n_clusters=number_of_clusters, linkage="complete"
        ).fit(X)

    """
        we have to make clusters with the linkage method and we have some other parameters for it 
    ...
    def Agglomerative_CompleteLink_cluster_Visitors_With_Parameters(
        self, X: pd.DataFrame, number_of_clusters: int, linkage: str
    ):
        return AgglomerativeClustering(
            n_clusters=number_of_clusters, linkage=linkage
        ).fit(X)
